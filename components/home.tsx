...
//           <div className="container mx-auto px-4 h-full flex flex-col justify-center items-center text-center pointer-events-auto">
//             <button
//               onClick={handleSignOut}
//               className="absolute top-4 right-4 z-10 flex items-center gap-2 px-4 py-2 bg-white text-gray-800 rounded-lg hover:bg-gray-100 transition-colors"
//             >
//               Sign Out
//             </button>
...
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { Search, ChefHat, PlusCircle, LogOut } from "lucide-react";
import RecipeCard from "../components/RecipeCard";
import "../app/globals.css";
import { HomePageProps } from "@/typescript/layout";
import { signOut } from "next-auth/react";
    ...
    const filtered = homePageData.recipes.filter((recipe) => {
      ...
      if (recipeData?.title) {
        ...
        const ingredientsMatch = recipeData.ingredients
          ...
          .split(",")
          .some((ingredient: string) =>
            ingredient.includes(lowercasedSearchTerm)
          );
